...
// El conmutador de tema es un segmented control de 2 opciones (claro/oscuro,
// ver ConmutadorTema.tsx — se sacó la opción "Sistema"): cada una es un boton
// role="radio" cuyo nombre accesible sale del atributo title (no hay texto
// visible, son iconos).
test.describe("Conmutador de tema", () => {
  test("selecciona claro/oscuro y persiste tras recargar", async ({ page }) => {
    ...
    await expect(page.getByRole("radio", { name: "Tema: Sistema" })).toHaveCount(0);
